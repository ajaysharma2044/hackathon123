...
| **— Event Operations (Event 1) —** | |
| [docs/event-ops/operational-architecture.md](docs/event-ops/operational-architecture.md) | **Start here.** The two orgs on one timeline; the operational layers; how ops IS the research sensor network |
| [docs/event-ops/prior-hackathons.md](docs/event-ops/prior-hackathons.md) | **The evidence base.** Sourced operational facts from MLH, HackMIT, Hack the North, TreeHacks, Cal Hacks, hackathon.guide — KNOWN vs our design choice |
| [docs/event-ops/org-chart-and-roles.md](docs/event-ops/org-chart-and-roles.md) · [role-correlations.md](docs/event-ops/role-correlations.md) | Every role + counts by size; the typed correlation graph (who depends on whom, the ops↔research interlock, criticality) |
| [docs/event-ops/logistics.md](docs/event-ops/logistics.md) · [run-of-show.md](docs/event-ops/run-of-show.md) | Venue/food/travel/check-in/wifi/safety + the 72-hour run sheet with the research cadence overlaid |
| [docs/event-ops/judging-system.md](docs/event-ops/judging-system.md) · [volunteers-and-crew.md](docs/event-ops/volunteers-and-crew.md) | Science-fair + finals (the judge formula, stack-ranking, sponsor conflicts); the volunteer/crew layer + the research firewall |
| **— Adaptive Team Performance + Rescue Engine —** | |
| [docs/team-rescue/adaptive-rescue-engine.md](docs/team-rescue/adaptive-rescue-engine.md) | **Start here.** Maximize useful contribution from every team; the state→intervention→outcome loop; **Preventable Blocked Minutes** as the Event 1 metric |
| [docs/team-rescue/team-state-model.md](docs/team-rescue/team-state-model.md) · [rescue-policy.md](docs/team-rescue/rescue-policy.md) | The team-state vector + archetype envelope + stall detection; the No-Dead-Team ladder, scope optimizer, diagnosis, quick-wins, pivot/submission/demo support |
| [docs/team-rescue/contribution-and-roles.md](docs/team-rescue/contribution-and-roles.md) · [event-control-and-waste.md](docs/team-rescue/event-control-and-waste.md) | Role coverage + ownership + critical path + rematch/merge (voluntary); the ORIE layer: bottleneck/waste map, fair allocation, adaptive schedule |
| [docs/team-rescue/firewalls-and-ethics.md](docs/team-rescue/firewalls-and-ethics.md) | No person scoring, no surveillance, the performance/research firewall, fairness floor, counterfactual discipline, preventable ≠ informative failure |
| [schema/](schema/) · [engine/](engine/) | Typed schema (core + economic + quant + [live-research](schema/006_live_research.sql)) + runnable engines: consent/temporal correctness, VOI kill-filter, beliefs, Monte Carlo, portfolio, calibration, event optimizer, value engines, company→offer matcher, **the Live Research OS kernel** (triggers, adaptive questions, adaptive sampling, mentor routing, question backlog, team trajectory, evidence graph, burden budget, intervention log), **the Event-Ops kernel** ([schema/007](schema/007_event_ops.sql) + staffing model, judging assignment / stack-rank, the role-correlation graph), **and the Team-Rescue kernel** ([schema/008](schema/008_team_rescue.sql) + team-state vector, rescue policy, critical path, live event control) — **234 passing checks across twelve suites** |
